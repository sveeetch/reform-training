...
Last updated: 2026-05-13, update 8
...
- On mobile, Start Workout can become a fixed bottom action only when the Session panel scrolls offscreen, no session is active, and the bottom workout actions are not visible. Finish Workout should not be a floating action.
...
- Each exercise has a `Фото` action that opens a dialog with either the user's saved machine photo or a generated/search preview plus Google Images and YouTube links.
- Exercise dialog supports uploading a custom machine photo. The app resizes it client-side and stores it in the exercise object so it syncs through Supabase with the shared program/session data.
...
- Sets can be added inside the active workout card. Active set rows have a minimal trash icon for deleting a set; swipe-to-delete was removed because the explicit button is clearer.
...
- Active workouts have two modes: `Список` shows all exercises, while `Шаги` shows one exercise at a time with previous/next navigation.
- Workout bottom actions include Start before a session, Finish during a session, and Skip Day as a single bottom action.
...
- Service worker cache currently must be bumped when deploying visible app changes. Current cache: `reform-training-v12`.
